...
# this environment includes a third block in the state information
@register_env("StackWithObstacle-v1", max_episode_steps=300)
class StackWithObstacleEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(
        ...
        self.env_x_limit = kwargs.pop("env_x_limit", [-0.2, -0.1]) # [-0.15, -0.05]
        self.env_y_limit = kwargs.pop("env_y_limit", [-0.15, 0.15])
        ...
        self.extra_margin = 0.05 # make it 0.05 if add obstacle
        self.xy_range = kwargs.pop("xy_range", 0.0)
        self.offset = kwargs.pop("offset", [0.0, 0.0])
        ## for obstacles:
        self.num_obstacles = kwargs.pop("num_obstacles", 0)  # set to >0 to put the obstacle between the 2 cubes
        self.block_halfsize = kwargs.pop("block_halfsize", [0.02, 0.02, 0.05])
        self.block_color = kwargs.pop("block_color", [0.0, 0.0, 1.0, 1])
        ## added for optional movement of objects B to the side:
        self.cubeB_offset = kwargs.pop("cubeB_offset", [0.15, 0.0]) # 0.12
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            if self.num_obstacles > 0: # put block in the middle of the path from cubeA to cubeB
            ...
            else: # put block far away so that it doesnt interfere (not considered as obstacle)
                hsC = torch.as_tensor(self.block_halfsize[0], device=self.device, dtype=torch.float32)
                xyzC = torch.zeros((b, 3), device=self.device)
                xyzC[:, :2] = torch.tensor([0.0, -0.9], device=self.device).expand(b, -1)
                xyzC[:, 2]  = hsC
                qsC = randomization.random_quaternions(b, lock_x=True, lock_y=True, lock_z=False)
                self.block.set_pose(Pose.create_from_pq(p=xyzC, q=qsC))
    ...
    def _get_obs_extra(self, info: dict):
        ...
        if "state" in self.obs_mode:
            obs.update(
                ...
                tcp_to_block_pos=self.block.pose.p - self.agent.tcp.pose.p,
