...
from mani_skill.envs.tasks import StackWithObstacleEnv
...
def solve(env: StackWithObstacleEnv, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    ...
    block_y = env.block.pose.p.cpu().numpy()[0][1]
    print("block_y", block_y)
    if block_y < -0.7: # obstacle doesnt interfere -> do straight line from cubeA to cubeB
    ...
    else: # obstacle is in the middle of the straight line from cubeA to cubeB
        approaching = np.array([0, 0, -1])
        target_closing = env.agent.tcp.pose.to_transformation_matrix()[0, :3, 1].cpu().numpy()
        grasp_info = compute_grasp_info_by_obb(
            obb,
