...
class Agent_RF(nn.Module):
    ...
    def get_action(self, obs_seq, fm_steps=None, use_heun=False):
        ...
        # track total path length and energy per batch element
        ...
        for i in range(fm_steps):
            ...
            t_b = t.expand(B)
            t_emb = self._embed_t(t_b)
            v1 = self.velocity_net(a, t_emb, global_cond=obs_cond)  # (B, pred_horizon, act_dim)
            ...
            if not use_heun:
                # energy using v1
                vel_sq = v1.reshape(B, -1).pow(2).sum(dim=1)   # ||v1||^2
                total_energy += 0.5 * vel_sq * dt

                a = a + v1 * dt
            else:
                # Heun: use average of v1 and v2
                a_euler = a + v1 * dt
                t_b_next = t_next.expand(B)
                t_emb_next = self._embed_t(t_b_next)
                v2 = self.velocity_net(a_euler, t_emb_next, global_cond=obs_cond)
                ...
                # energy using v_avg
                vel_sq = v_avg.reshape(B, -1).pow(2).sum(dim=1)   # ||v_avg||^2
                total_energy += 0.5 * vel_sq * dt
        ...
        # return action sequence, straightness metric, and total energy
        return a_out, straightness, total_energy
...
class Agent_RFpi0(nn.Module):
    ...
    def get_action(self, obs_seq, fm_steps=None, use_heun=False):
        ...
        for i in range(fm_steps):
            ...
            if not use_heun:
                # energy using v1
                vel_sq = v1.view(B, -1).pow(2).sum(dim=1)     # ||v1||^2
                total_energy += 0.5 * vel_sq * dt

                a = a + v1 * dt
            else:
                # Heun: average of v1 and v2
                a_euler = a + v1 * dt
                t_b_next = t_next.expand(B)
                t_emb_next = self._embed_t(t_b_next)
                v2 = self.velocity_net(a_euler, t_emb_next, global_cond=obs_cond)
                ...
                # energy using v_avg
                vel_sq = v_avg.view(B, -1).pow(2).sum(dim=1)  # ||v_avg||^2
                total_energy += 0.5 * vel_sq * dt
        ...
        # return action, straightness metric, and total energy
        return a_out, straightness, total_energy
...
class Agent_CFM(nn.Module):
    ...
    def get_action(self, obs_seq, fm_steps=None, use_heun=False):
        ...
        # track total path length and total energy per batch element
        path_length = torch.zeros(B, device=device)
        total_energy = torch.zeros(B, device=device)
        ...
        for i in range(fm_steps):
            ...
            if not use_heun:
                ...
                # Heun: average velocity between v1 and v2
                a_euler = a + v1 * dt
                t_b_next = t_next.expand(B)
                t_emb_next = self._embed_t(t_b_next)
                v2 = self.velocity_net(a_euler, t_emb_next, global_cond=obs_cond)
...
class Agent_CFMpi0(nn.Module):
    ...
    def get_action(self, obs_seq, fm_steps=None, use_heun=False):
        ...
        return a_out, straightness, total_energy  # optionally clamp a_out if needed
...
class Agent_DP(nn.Module): # standard diffusion policy Agent class
    ...
    def get_action(self, obs_seq):
        ...
        # initialize from Gaussian noise (this is the *only* source of randomness)
        ...
        for k in timesteps:
            ...
            # DDIM deterministic step: eta=0 → no diffusion noise
            out = self.noise_scheduler.step(
                model_output=noise_pred,
                timestep=k,
                sample=noisy_action_seq,
                eta=0.0,               # <- key: deterministic
            )
            noisy_action_seq = out.prev_sample

            # step displacement and energy (if you still want those)
            step_disp = noisy_action_seq - x_prev
            step_disp_flat = step_disp.reshape(B, -1)

            path_length += step_disp_flat.norm(dim=1)

            vel_sq = (step_disp_flat / dt).pow(2).sum(dim=1)
            total_energy += 0.5 * vel_sq * dt

        xT = noisy_action_seq

        straight_dist = (xT - x0).reshape(B, -1).norm(dim=1)
        eps = 1e-8
        straightness = straight_dist / (path_length + eps)

        start = self.obs_horizon - 1
        end = start + self.act_horizon
        actions = noisy_action_seq[:, start:end]

        return actions, straightness, total_energy

    @torch.no_grad()
    def get_action2(self, obs_seq):
        # obs_seq: (B, obs_horizon, obs_dim)
        B = obs_seq.shape[0]
        device = obs_seq.device
        ...
        with torch.no_grad():
            ...
            # track path length and energy per batch
            path_length = torch.zeros(B, device=device)
            total_energy = torch.zeros(B, device=device)

            timesteps = self.noise_scheduler.timesteps
            K = len(timesteps)
            dt = 1.0 / float(K)  # assume uniform time grid in [0, 1]
            ...
            for k in timesteps:
                ...
                step_disp_flat = step_disp.reshape(B, -1)      # (B, D)

                # path length: sum ||Δx||
                path_length += step_disp_flat.norm(dim=1)

                # energy: 0.5 * ||v||^2 * dt, with v ≈ Δx / dt
                vel_sq = (step_disp_flat / dt).pow(2).sum(dim=1)  # ||v||^2
                total_energy += 0.5 * vel_sq * dt
...
def evaluate_ckpt(agent, args, device, modelname, fm_steps):
    ...
    t0 = time.time()
    eval_metrics, straightness_mean, energy_mean = evaluate_energy(
