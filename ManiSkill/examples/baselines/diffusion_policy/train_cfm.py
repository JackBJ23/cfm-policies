...
class Args:
    ...
    agent_type: str = 'DP'
    """agent type: "OTCFM", "RF" for rectified flow, "CFM" for standard flow matching, or "DP" for diffusion policy"""
...
class Agent_OTCFM(nn.Module):
    def __init__(self, env, args):
        ...
        # --- Velocity field network (reuse your ConditionalUnet1D backbone) ---
        ...
        # keep a scale for continuous time embedding if your UNet expects a "timestep"
        self.t_embed_scale = 999.0  # maps t∈[0,1] to [0,999] like diffusion steps

        # sampler steps
        self.fm_steps = args.fm_steps

        # OT-CFM knobs (feel free to promote to args)
        self.ot_eps = 0.2          # entropic regularization (bigger = softer, more stable)
        self.ot_iters = 50         # Sinkhorn iterations
        self.cond_sim_thresh = 0.7 # only match within similar obs contexts (initially tried 0.95, too high)

    def _embed_t(self, t_b):  # t_b in [0,1], shape (B,)
        return (t_b * self.t_embed_scale).float()  # e.g., t_embed_scale = 999.0
    
    @staticmethod
    def _pairwise_sq_dist(x, y):
        """x: (B,D), y: (B,D) -> (B,B) with ||x_i - y_j||^2"""
        x2 = (x**2).sum(-1, keepdim=True)        # (B,1)
        y2 = (y**2).sum(-1).unsqueeze(0)         # (1,B)
        xy = x @ y.t()                            # (B,B)
        return x2 - 2*xy + y2
    
    @staticmethod
    def _cosine_sim(a, b, eps=1e-8):
        """a: (B,D), b: (B,D) -> (B,B) cosine similarity matrix"""
        a = a / (a.norm(dim=-1, keepdim=True) + eps)
        b = b / (b.norm(dim=-1, keepdim=True) + eps)
        return a @ b.t()

    def _sinkhorn(self, cost, eps, iters, mask=None):
        """
        cost: (B,B) non-negative cost
        eps : entropic reg
        mask: (B,B) boolean, False disallowed (set to large cost internally)
        returns soft transport plan P (B,B), approx doubly-stochastic
        """
        B = cost.size(0)
        if mask is not None:
            # Forbid cross-context matches by inflating cost
            big = 1e6
            cost = cost + (~mask) * big
        ...
        if mask is not None:
            K = K * mask  # zero out disallowed entries exactly
        K = K.clamp_min(1e-12)
        u = torch.ones(B, device=cost.device) / B
        v = torch.ones(B, device=cost.device) / B
        ...
        P = torch.diag(u) @ K @ torch.diag(v)  # (B,B)
        # Optional: project tiny negatives to 0 due to numeric noise
        return P.clamp_min(0.0)

    def _ot_match_prior(self, a0, a1, obs_cond):
        """
        a0: (B, L, A) data actions
        a1: (B, L, A) iid prior samples
        obs_cond: (B, D) conditioning vectors; used to restrict matches

        returns a1_matched: (B, L, A)
        """
        B, L, A = a0.shape
        device = a0.device

        # Build features for costs: flatten sequences
        a0_flat = a0.reshape(B, -1)
        a1_flat = a1.reshape(B, -1)

        # (optional) standardize to improve OT numerics
        with torch.no_grad():
            mean = torch.cat([a0_flat, a1_flat], dim=0).mean(dim=0, keepdim=True)
            std  = torch.cat([a0_flat, a1_flat], dim=0).std(dim=0, keepdim=True) + 1e-6
            x = (a0_flat - mean)/std
            y = (a1_flat - mean)/std

        # Mask by conditioning similarity so we don't couple across very different contexts
        with torch.no_grad():
            sim = self._cosine_sim(obs_cond, obs_cond)             # (B,B)
            mask = sim > self.cond_sim_thresh                      # boolean (B,B)

        # Quadratic cost
        with torch.no_grad():
            C = self._pairwise_sq_dist(x, y)                       # (B,B)

            # Soft OT via Sinkhorn, returns P ~ doubly stochastic
            P = self._sinkhorn(C, eps=self.ot_eps, iters=self.ot_iters, mask=mask)  # (B,B)

            # Weighted sum of prior samples -> matched prior for each row of a0
            # Broadcast P over (L,A) by multiplying on batch axis
            a1_matched = torch.einsum('ij,jla->ila', P, a1)        # (B,L,A)
    ...
    def compute_loss(self, obs_seq, action_seq):
        ...
        a0 = action_seq                                        # (B, L, A)
        a1_iid = torch.randn_like(action_seq)                  # (B, L, A)
        # --- OT pairing within the batch (masked by similar conditioning) ---
        a1 = self._ot_match_prior(a0, a1_iid, obs_cond)        # (B, L, A)

        # Sample continuous FM time
        t = torch.rand(B, 1, 1, device=device)                 # (B,1,1)
        a_t = (1.0 - t) * a0 + t * a1

        # Rectified-flow target velocity (linear path)
        target_v = a1 - a0

        # Predict velocity
        t_b = t.squeeze(-1).squeeze(-1)                        # (B,)
        t_emb = self._embed_t(t_b)                             # float (B,)
        pred_v = self.velocity_net(a_t, t_emb, global_cond=obs_cond)
    ...
    def get_action(self, obs_seq, fm_steps=None, use_heun=True):
        ...
        # start from prior at t=1
        a = torch.randn((B, self.pred_horizon, self.act_dim), device=device)

        # integrate t: 1 -> 0
        t_grid = torch.linspace(1.0, 0.0, fm_steps + 1, device=device)
        ...
        # slice to action horizon and clamp to action space
...
class Agent_RF(nn.Module):
    ...
    def compute_loss(self, obs_seq, action_seq):
        ...
        a0 = action_seq
        a1 = torch.randn_like(action_seq)  # base endpoint

        t = torch.rand(B, 1, 1, device=device)  # (B,1,1)
        a_t = (1.0 - t) * a0 + t * a1

        target_v = a1 - a0  # rectified flow target
        # (B,) time for the network
        t_b = t.squeeze(-1).squeeze(-1)               # (B,)
        t_emb = self._embed_t(t_b)                    # (B,)

        pred_v = self.velocity_net(a_t, t_emb, global_cond=obs_cond)
        # the velocity target_v matches a0 at t=0 to a1 at t=1 (goes from a0 (action seq) to a1 (noise))
        return F.mse_loss(pred_v, target_v)
...
class Agent_CFM(nn.Module): ## to do
    def __init__(self, env, args):
        super().__init__()
        self.obs_horizon = args.obs_horizon
        self.act_horizon = args.act_horizon
