...
if __name__ == "__main__":
    ...
    # OOD-pos variant (object positions shifted) (shifted along Y axis to the middle)
    env_kwargs_ood_pos2 = dict(
        control_mode=args.control_mode,
        reward_mode="sparse",
        obs_mode="state",
        render_mode="rgb_array",
        human_render_camera_configs=dict(shader_pack="default"),
        cubeA_color=[1, 0, 0, 1], env_y_limit=[-0.05, 0.05], xy_range=0.00, offset=[0.0, 0.0],
        max_episode_steps=args.max_episode_steps,
    )
    # OOD-pos variant (cubeB position shifted to the left: moved to y=-0.15)
    env_kwargs_ood_pos3 = dict(
        control_mode=args.control_mode,
        reward_mode="sparse",
        obs_mode="state",
        render_mode="rgb_array",
        human_render_camera_configs=dict(shader_pack="default"),
        cubeA_color=[1, 0, 0, 1], xy_range=0.00, offset=[0.0, 0.15], cubeB_offset=[0.0, -0.3],
        max_episode_steps=args.max_episode_steps,
    )
    # OOD-color (make cubeB blue)
    env_kwargs_ood_color = dict(
        control_mode=args.control_mode,
        reward_mode="sparse",
        obs_mode="state",
        render_mode="rgb_array",
        human_render_camera_configs=dict(shader_pack="default"),
        cubeA_color=[1, 0, 0, 1], cubeB_color=[0, 0, 1, 1], xy_range=0.00, offset=[0.0, 0.15],
        max_episode_steps=args.max_episode_steps,
    )
    # OOD-size (make cubeB smaller and A smaller)
    env_kwargs_ood_size = dict(
        control_mode=args.control_mode,
        reward_mode="sparse",
        obs_mode="state",
        render_mode="rgb_array",
        human_render_camera_configs=dict(shader_pack="default"),
        cubeA_halfsize=0.01, cubeB_halfsize=0.01, xy_range=0.00, offset=[0.0, 0.15],
        max_episode_steps=args.max_episode_steps,
    )
    ...
    if args.capture_video:
        ...
        video_dir_ood2 = f"runs/eval_ood_pos2/videos_{args.folderout}"
        video_dir_ood3 = f"runs/eval_ood_pos3/videos_{args.folderout}"
        video_dir_color = f"runs/eval_ood_color/videos_{args.folderout}"
        video_dir_size = f"runs/eval_ood_size/videos_{args.folderout}"
        os.makedirs(video_dir, exist_ok=True)
        os.makedirs(video_dir_ood1, exist_ok=True)
        os.makedirs(video_dir_ood2, exist_ok=True)
        os.makedirs(video_dir_ood3, exist_ok=True)
        os.makedirs(video_dir_color, exist_ok=True)
        os.makedirs(video_dir_size, exist_ok=True)
    ...
    env_ood_pos2 = make_eval_envs(
        args.env_id, args.num_eval_envs, args.sim_backend,
        env_kwargs_ood_pos2, other_kwargs, video_dir=video_dir_ood2
    )
    # Evaluate (OOD-pos 2)
    t0 = time.time()
    eval_metrics_ood2 = evaluate(
        args.num_eval_episodes, agent, env_ood_pos2, device, args.sim_backend
    )
    elapsed_ood2 = time.time() - t0
    env_ood_pos2.close()
    print(f"\n=== OOD Position Evaluation 2 ===")
    print(f"Evaluated {len(eval_metrics_ood2.get('success_at_end', []))} episodes in {elapsed_ood2:.1f}s")
    # print per-metric mean
    for k, arr in eval_metrics_ood2.items():
        try:
            val = float(np.mean(arr))
        except Exception:
            val = arr
    ...
    env_ood_pos3 = make_eval_envs(
        args.env_id, args.num_eval_envs, args.sim_backend,
        env_kwargs_ood_pos3, other_kwargs, video_dir=video_dir_ood3
    )
    # Evaluate (OOD-pos 3)
    t0 = time.time()
    eval_metrics_ood3 = evaluate(
        args.num_eval_episodes, agent, env_ood_pos3, device, args.sim_backend
    )
    elapsed_ood3 = time.time() - t0
    env_ood_pos3.close()
    print(f"\n=== OOD Position Evaluation 3 ===")
    print(f"Evaluated {len(eval_metrics_ood3.get('success_at_end', []))} episodes in {elapsed_ood3:.1f}s")
    # print per-metric mean
    for k, arr in eval_metrics_ood3.items():
        try:
            val = float(np.mean(arr))
        except Exception:
            val = arr
    ...
    env_ood_color = make_eval_envs(
        args.env_id, args.num_eval_envs, args.sim_backend,
        env_kwargs_ood_color, other_kwargs, video_dir=video_dir_ood3
    )
    # Evaluate (OOD-color)
    t0 = time.time()
    eval_metrics_ood_color = evaluate(
        args.num_eval_episodes, agent, env_ood_color, device, args.sim_backend
    )
    elapsed_ood_color = time.time() - t0
    env_ood_color.close()
    print(f"\n=== OOD Color Evaluation ===")
    print(f"Evaluated {len(eval_metrics_ood_color.get('success_at_end', []))} episodes in {elapsed_ood_color:.1f}s")
    # print per-metric mean
    for k, arr in eval_metrics_ood_color.items():   
        try:
            val = float(np.mean(arr))
        except Exception:
            val = arr
    ...
    env_ood_size = make_eval_envs(
        args.env_id, args.num_eval_envs, args.sim_backend,
        env_kwargs_ood_size, other_kwargs, video_dir=video_dir_ood3
    )
    # Evaluate (OOD-size)
    t0 = time.time()
    eval_metrics_ood_size = evaluate(
        args.num_eval_episodes, agent, env_ood_size, device, args.sim_backend
    )
    elapsed_ood_size = time.time() - t0
    env_ood_size.close()
    print(f"\n=== OOD Size Evaluation ===")
    print(f"Evaluated {len(eval_metrics_ood_size.get('success_at_end', []))} episodes in {elapsed_ood_size:.1f}s")
    # print per-metric mean
    for k, arr in eval_metrics_ood_size.items():
        try:
            val = float(np.mean(arr))
        except Exception:
            val = arr
