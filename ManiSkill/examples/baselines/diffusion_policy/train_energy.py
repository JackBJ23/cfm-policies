...
class Agent_DP(nn.Module): # standard diffusion policy Agent class
    def __init__(self, env, args):
        ...
        self.sample_scheduler = DDIMScheduler.from_config(self.noise_scheduler.config)
    ...
    def get_action(self, obs_seq):
        ...
        # Set inference timesteps for DDIM
        self.sample_scheduler.set_timesteps(self.num_diffusion_iters)
        ...
        for k in timesteps:
            ...
            out = self.sample_scheduler.step(
...
if __name__ == "__main__":
    ...
    def evaluate_and_save_best(iteration):
        if iteration % args.eval_freq == 0:
            ...
            # ---- NEW: log straightness ----
            writer.add_scalar("eval/straightness_mean", straightness_mean, iteration)
            print(f"straightness_mean: {straightness_mean:.4f}")

            writer.add_scalar("eval/energy_mean", energy_mean, iteration)
            print(f"energy_mean: {energy_mean:.4f}")

            # if you *also* want explicit wandb logging (not necessary if sync_tensorboard=True):
            # if args.track:
            #     wandb.log({"eval/straightness_mean": straightness_mean}, step=iteration)
