...
python -m mani_skill.examples.motionplanning.panda.run -e "StackWithObstacle-v1" \
    -n ${demos1} -o state --num-procs 10 --traj-name ${name} \
    --add-obstacle --save-video --record-dir demos_${name}

python -m mani_skill.trajectory.replay_trajectory --traj_path /n/home08/jackbjed/scaling-diffusion-policy/ManiSkill/demos_${name}/StackWithObstacle-v1/motionplanning/${name}.h5 \
